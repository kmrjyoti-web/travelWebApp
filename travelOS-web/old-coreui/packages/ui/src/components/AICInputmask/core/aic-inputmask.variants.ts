/**
 * AICInputmask size, variant, shape, and state style maps.
 * Framework-agnostic — returns Tailwind class strings with CSS custom properties.
 *
 * Matches AICTextbox / AICNumber styling pattern.
 */
...
export const sizeStyles: Record<AICInputmaskSize, string> = {
  sm: "h-8 text-sm",
  md: "h-10 text-sm",
  lg: "h-12 text-base",
};
...
export const variantStyles: Record<AICInputmaskVariant, string> = {
  outlined:
...
export const shapeStyles: Record<AICInputmaskShape, string> = {
...
export const stateStyles: Record<AICInputmaskState, string> = {
  default:
