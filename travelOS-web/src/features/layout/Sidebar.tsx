...
export const MENU_DATA: MenuNode[] = [
  ...
  { label: "Itinerary", icon: <Map size={16} />, href: "/itinerary" },
  {
    label: "Master",
...
function MenuItem({
  ...
  if (href) {
    return (
      <Link href={href} className={className} style={{ textDecoration: 'none', color: 'inherit' }} title={!isOpen ? label : undefined}>
        {content}
      </Link>
    );
  }
  ...
  return (
    <div className={className} title={!isOpen ? label : undefined}>
      {content}
    </div>
  );
}
